...
class PRRepository:
    ...
    async def insert_pr(
        ...
        await self.db.execute(Q.INSERT_PR, (
            chatbot_id, repo_name, pr_number, pr_url, pr_title,
            pr_author, pr_created_at, pr_merged, status, bq_json,
            bot_reviewed_at,
        ))
    ...
    async def lock_pr(self, pr_id: int, worker_id: str, lock_timeout_minutes: int = 30) -> bool:
        ...
        stale_cutoff = (datetime.now(timezone.utc) - timedelta(minutes=lock_timeout_minutes)).isoformat()
    ...
    async def update_metadata(
        ...
        pr_created_at: str | None, pr_merged: bool | None
    ) -> None:
    ...
    async def insert_analysis(
        ...
        await self.db.execute(Q.INSERT_LLM_ANALYSIS, (
            pr_id, chatbot_id,
